/**
 * BEA detail-level I-O code labels from official AllTablesIO
 * “IOUse_Before_Redefinitions_PRO_2017_Detail.xlsx”, sheet 2017,
 * column headers (row 5–6) and commodity rows (col A–B).
 * Supplemental T00… codes plus VABAS and VAPRO cover variants missing from that extract.
 *
 * Source: https://apps.bea.gov/industry/iTables%20Static%20Files/AllTablesIO.zip
 */

export const BEA_IO_DETAIL_NAMES: Record<string, string> = {
  ...
  "T018": "Total (supplementary I-O row T018)",
  "T019": "Total (supplementary I-O row T019)",
  ...
  "VABAS": "Value added — basic prices (supplementary row)",
  "VAPRO": "Value added — producers' prices (supplementary row)",
  ...
  "T00SUB": "Subtotal — other secondary products (T00SUB)",
  "T00TOP": "Top-level total — other products (T00TOP)",
  "V00100": "Compensation of employees",
  "V00200": "Taxes on production and imports, less subsidies",
  "V00300": "Gross operating surplus",
  "T00OSUB": "Subtotal — other specialized industries (T00OSUB)",
  "T00OTOP": "Top-level total — other specialized (T00OTOP)",
};

export function lookupBeaIoDetailName(code: string): string | undefined {
  const c = code.trim();
  const direct = BEA_IO_DETAIL_NAMES[c];
  if (direct) return direct;
  if (/^F\d{2}[A-Z0-9]00$/.test(c)) {
    return BEA_IO_DETAIL_NAMES[c.slice(0, -2)];
  }
  return undefined;
}
